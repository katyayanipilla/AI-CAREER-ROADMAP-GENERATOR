...
def update_streak(username):
    ...
    c.execute("SELECT streak, last_active, max_streak FROM users WHERE username=?", (username,))
    data = c.fetchone()

    if not data:
        return

    streak, last_active, max_streak = data
    ...
    if streak > (max_streak or 0):
        max_streak = streak

    c.execute("""
        UPDATE users
        SET streak=?, last_active=?, max_streak=?
        WHERE username=?
    """, (streak, today.isoformat(), max_streak, username))
